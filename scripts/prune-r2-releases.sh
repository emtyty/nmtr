#!/usr/bin/env bash
#
# Prune old nmtr release artifacts from the R2 (S3-compatible) bucket,
...
#
# Versioned artifacts eligible for deletion (carry an X.Y.Z in the key):
#   nmtr-Setup-<ver>.exe[.blockmap]       (Windows NSIS installer)
#   nmtr-<ver>-x64.exe                    (Windows portable)
#   nmtr-<ver>.dmg[.blockmap]             (macOS x64 disk image)
#   nmtr-<ver>-arm64.dmg[.blockmap]       (macOS arm64 disk image)
#   nmtr-<ver>-mac.zip[.blockmap]         (macOS x64 zip   — used by updater)
#   nmtr-<ver>-arm64-mac.zip[.blockmap]   (macOS arm64 zip — used by updater)
#
# Unversioned keys are NEVER touched (no extractable X.Y.Z):
#   latest.yml / latest-mac.yml           (update manifests, overwritten each build)
#
