...
export function IconNav({ activeView, onNavigate }: IconNavProps): React.JSX.Element {
  ...
  return (
    <div className="w-16 bg-canvas-subtle border-r border-border-default flex flex-col items-center py-3 gap-1 flex-shrink-0">
      {items.map((item) => {
        ...
        return (
          <button
            ...
            onClick={onClick}
            title={item.label}
            className={`flex flex-col items-center justify-center gap-1 w-12 h-12 rounded-lg transition-colors ${
              isActive
